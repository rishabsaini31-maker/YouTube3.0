...
export async function GET(request: NextRequest) {
  try {
    ...
    const filter = searchParams.get('filter') || 'all' // all | videos | channels

    if (!query.trim()) {
      return NextResponse.json({ videos: [], channels: [], total: 0, page, pageSize, totalVideos: 0, totalChannels: 0 })
    }
